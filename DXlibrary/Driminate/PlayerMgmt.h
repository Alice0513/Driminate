...
private:
	...
	int _shadowModel;
	VECTOR _shadowPos = VGet(0.0f, 1.5f, 0.0f);
	...
	int _waterDamage = 1;     // 水のダメージ
	int _ohDamage = 1;        // OHのダメージ

	bool _hitFlg = false;
	bool _nowMoveFlg[4] = { false, false, false, false };
	float _timer = 0.0;
public:
	PlayerMgmt();
	void ValueReset();
