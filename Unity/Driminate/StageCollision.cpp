...
StageCollision::StageCollision()
{
	...
	_spaceModel[0] = MV1LoadModel("Model/Stage/Water.mv1");
}
...
void StageCollision::Update(int num, VECTOR playerPos)
{
	...
	_playerMgmt->CollProcWater(HitCheck(_spaceModel[0], playerPos, &offset), offset);
	...
	// モデル描画
	MV1SetPosition(_spaceModel[0], _pos);
	MV1SetScale(_spaceModel[0], VGet(5.0f, 5.0f, 5.0f));
	MV1DrawModel(_spaceModel[0]);
}
...
{
	int root_index = MV1SearchFrame(_stageModel[0], "Stage");
	traverse(_stageModel[0], root_index, "Stage");
	root_index = MV1SearchFrame(_spaceModel[0], "Stage");
	traverse(_spaceModel[0], root_index, "Stage");
}
